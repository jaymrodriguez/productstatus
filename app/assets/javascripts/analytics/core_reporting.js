...
document.getElementById('start-date').value = lastNDays(30);
document.getElementById('end-date').value = lastNDays(1);
...
function buildDataFromResponse(resp) {
	...
	for (var id in resp) {
		var results = resp[id].result;
		if (results.rows && results.rows.length) {
			name += resp[id].id.makeKey();
			dataset += '{ "key" : "'+resp[id].id+'", "values" : [';
		    // Put cells in table.
		    for (var i = 0; i<results.rows.length; ++i) {
				var rawDate = results.rows[i][0].slice(0,4)+
					'-'+results.rows[i][0].slice(4,6)+
					'-'+results.rows[i][0].slice(6,8);
				var date = Date.parse(rawDate);
		    	dataset +="["+date+","+results.rows[i][1]+"],";
		    }
 ...
 */
function returnJSON(results,title) {
  if (!results.code) {
	console.log(title);
	rowsToJSON(results);
	console.log(results);
...
/**
 * Prints all the column headers and rows of data as JSON.
 * @param {Object} results The object returned from the API.
 */
function rowsToJSON(results) {
...
// Utility method to turn a string into an ID

String.prototype.makeKey = function () {
        key = this.trim().split(' ').join('');
        key = key.split('(').join('');
        key = key.split(')').join('');
        key = key.split('$').join('');
        key = key.split('%').join('percent');
        key = key.split('+').join('');
        key = key.split('&').join('');
        key = key.split('/').join('');
        key = key.split('.').join('');
